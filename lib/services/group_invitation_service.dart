...
class GroupInvitationService {
  ...
  static String? get _userDisplayName => _auth.currentUser?.displayName ?? '匿名ユーザー';
  ...
  /// 招待コードの詳細情報を取得
  static Future<Map<String, dynamic>?> getInvitationInfo(String invitationCode) async {
    try {
  ...
  /// グループの招待コード一覧を取得
  static Future<List<Map<String, dynamic>>> getGroupInvitationCodes(String groupId) async {
    if (_uid == null) throw Exception('未ログイン');
  ...
  /// 招待コードの統計情報を取得
  static Future<Map<String, dynamic>> getInvitationCodeStats(String invitationCode) async {
    try {
      final invitationInfo = await getInvitationInfo(invitationCode);
      if (invitationInfo == null) {
        return {};
  ...
  static Future<Map<String, dynamic>?> getGroupInfoFromInvitationCode(String invitationCode) async {
    try {
      final invitationInfo = await getInvitationInfo(invitationCode);
