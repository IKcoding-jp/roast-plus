...
class RoastSchedulerTabState extends State<RoastSchedulerTab>
  ...
  Future<void> _generateSchedule() async {
    ...
    if (_amStart != null && _pmStart != null) {
      ...
      _futureResult!.then((data) async {
        try {
          await UserSettingsFirestoreService.saveSetting('roastSchedule_autoGenerated', true);
        } catch (e) {
          print('自動生成フラグ保存エラー: $e');
        }
      });
  ...
  Future<void> _saveInputState() async {
    try {
      ...
      final beans = provider.beans;
      final beansJson = beans.map((b) => b.toJson()).toList();

      await UserSettingsFirestoreService.saveMultipleSettings({
        'roastSchedule_beans': beansJson,
        'roastSchedule_amStart': _amStart?.format(context),
        'roastSchedule_pmStart': _pmStart?.format(context),
      });
