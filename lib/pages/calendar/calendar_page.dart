...
class _CalendarPageState extends State<CalendarPage> {
  ...
  Map<String, dynamic>? _assignmentMembers;
  ...
  Future<Map<String, dynamic>?> _loadRoastScheduleForDate(
    ...
    try {
      ...
      print('指定日付のローストスケジュール読み込みエラー: $e');
      return null;
    }
  }

  // 指定した日付のTODOリストを取得
  Future<List<Map<String, dynamic>>?> _loadTodoListForDate(
    String dateKey,
  ) async {
    try {
      final firestore = FirebaseFirestore.instance;
  ...
  Widget _buildDripPackSection(ThemeSettings themeSettings) {
    final totalCount = _dripPackRecords.fold<int>(
      0,
      (sum, record) => sum + (record['count'] as int),
    );
                          ...
                          child: Row(
                            children: [
                              Icon(
                                ...
                                color: isCompleted
                                    ? Colors.green
                                    : themeSettings.iconColor.withOpacity(0.5),
                              ),
                              SizedBox(width: 4),
                              Text(
                                _getStageName(stage),
                                style: TextStyle(
                                  color: isCompleted
                                      ...
                                      : themeSettings.fontColor1.withOpacity(
                                          0.6,
                                        ),
                                  fontSize: 12,
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isSelected
                ? themeSettings.buttonColor
                : themeSettings.iconColor.withOpacity(0.3),
            width: 1,
          ),
        ),
        child: Column(
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                color: isSelected
                    ? themeSettings.fontColor2
                    : themeSettings.fontColor1.withOpacity(0.7),
                fontSize: 10,
