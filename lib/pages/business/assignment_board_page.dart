...
class AssignmentBoardState extends State<AssignmentBoard> {
  ...
  void setAssignmentMembersFromFirestore(Map<String, dynamic> members) {
    debugPrint('AssignmentBoard: setAssignmentMembersFromFirestore 呼び出し');
    debugPrint('AssignmentBoard: 受信データ: $members');
    if (mounted) {
      setState(() {
      ...
      _groupAssignmentSubscription =
          GroupDataSyncService.watchGroupAssignmentBoard(group.id).listen((
            ...
            if (groupAssignmentData != null) {
              ...
              debugPrint('AssignmentBoard: グループ担当表データが空です - デフォルトデータを読み込み');
  ...
  Future<void> _loadDefaultData() async {
    try {
      debugPrint('AssignmentBoard: デフォルトデータ読み込み開始');
      ...
      debugPrint('AssignmentBoard: デフォルトデータ読み込み完了');
    } catch (e) {
      debugPrint('AssignmentBoard: デフォルトデータ読み込みエラー: $e');
    }
  }
  ...
  Future<void> _loadState() async {
    ...
    if (groupProvider.groups.isNotEmpty) {
      debugPrint('AssignmentBoard: グループ状態 - グループデータ監視を待機中');
      ...
      Future.delayed(Duration(seconds: 5), () {
        if (mounted && _isLoading) {
          debugPrint('AssignmentBoard: グループデータ読み込みタイムアウト - ローディング状態を解除');
          setState(() {
            _isLoading = false;
          });
        }
    ...
    debugPrint('AssignmentBoard: Firebaseデータを使用します');
    ...
    try {
      ...
      debugPrint('AssignmentBoard: Firebaseデータ読み込みエラー: $e');
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
